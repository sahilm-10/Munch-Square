...
<body>
    ...
    <?php
        $id = $_GET['restid'];
        $sql = "SELECT * FROM `restaurants` WHERE restId = $id";
        $result = mysqli_query($conn, $sql);
        while($row = mysqli_fetch_assoc($result)){
            $catname = $row['restName'];
            $catdesc = $row['restDesc'];
        }
    ?>
    ...
    <div class="container my-3" id="cont">
        ...
        <?php
            $id = $_GET['restid'];
            $sql = "SELECT * FROM `list` WHERE restCategorieId = $id";
            $result = mysqli_query($conn, $sql);
            $noResult = true;
            while($row = mysqli_fetch_assoc($result)){
                $noResult = false;
                $restId = $row['restId'];
                $restName = $row['resrName'];
                $restPrice = $row['restPrice'];
                $restDesc = $row['restDesc'];
            
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                        <div class="card" style="width: 18rem;">
                            <img src="img/rest-'.$restId. '.jpg" class="card-img-top" alt="image for this pizza" width="249px" height="270px">
                            <div class="card-body">
                                <h5 class="card-title">' . substr($restName, 0, 20). '...</h5>
                                <h6 style="color: #ff0000">Rs. '.$restPrice. '/-</h6>
                                <p class="card-text">' . substr($restDesc, 0, 29). '...</p>   
                                <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE restId = '$restId' AND `userId`='$userId'";
                            ...
                            echo '</form>                            
                                <a href="viewPizza.php?pizzaid=' . $restId . '" class="mx-2"><button class="btn btn-primary">Quick View</button></a> 
                                </div>
                            </div>
                        </div>
                    </div>';
